...
df = pd.read_excel('D:/研究生/专业学习/灰色预测/要写/锂电池/NASAlidianchi/NASA.xlsx', sheet_name='B7')
# 1.这里将 x1 作为特征序列 x2,x3,x4,x5作为相关因素序列。19个拟合，5个预测
X1 = df.iloc[:134, 0].tolist()
X2 = df.iloc[:134, 1].tolist()
X3 = df.iloc[:134, 2].tolist()
#测试预测值数据 自变量序列
XC2 = df.iloc[134:167, 1].tolist()
XC3 = df.iloc[134:167, 2].tolist()
#B7
#orders = [1,     0.74406203, 1 ]
#lamda = [0.98470547, 0.001,  1.]
orders = [1.00998586, 1.00998586, 1.00998586]
lamda = [0.99093091, 0.99093091, 0.99093091]

#预测的个数和真实值
h = 33
forezsz = df.iloc[134:167, 0].tolist()
allX1 = df.iloc[:, 0].tolist()
N = 2

'''df = pd.read_excel('D:/研究生/专业学习/灰色预测/要写/锂电池/test/NASAtest.xlsx', sheet_name='B51')
# 1.这里将 x1 作为特征序列 x2,x3,x4,x5作为相关因素序列。19个拟合，5个预测
...
lamda = [0.99093091, 0.99093091, 0.99093091]
#B51
orders = [1.00998586, 1.00998586, 1.03898586]
lamda = [0.95593091, 0.99093091, 0.99293091]
