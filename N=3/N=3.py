...
'''df = pd.read_excel('D:/研究生/专业学习/灰色预测/要写/锂电池/test/NASAtest.xlsx', sheet_name='B49')
# 1.这里将 x1 作为特征序列 x2,x3,x4,x5作为相关因素序列。19个拟合，5个预测
X1 = df.iloc[:19, 0].tolist()
X2 = df.iloc[:19, 1].tolist()
X3 = df.iloc[:19, 2].tolist()
#测试预测值数据 自变量序列
XC2 = df.iloc[19:24, 1].tolist()
XC3 = df.iloc[19:24, 2].tolist()

#B49
orders = [0.99984818, 0.03949673, 0.6759272]
lamda = [0.63156918, 0.001,      0.5134699]
#B51
orders = [1.03123798, 1.02981785, 1.05]
lamda = [0.75557473, 0.47864249, 0.77532572]

#预测的个数和真实值
h = 5
forezsz = df.iloc[19:24, 0].tolist()
allX1 = df.iloc[:, 0].tolist()
N = 3'''

df = pd.read_excel('D:/研究生/专业学习/灰色预测/要写/锂电池/NASAlidianchi/NASA.xlsx', sheet_name='B5')
# 1.这里将 x1 作为特征序列 x2,x3,x4,x5作为相关因素序列。19个拟合，5个预测
X1 = df.iloc[:133, 3].tolist()
X2 = df.iloc[:133, 1].tolist()
X3 = df.iloc[:133, 2].tolist()
#测试预测值数据 自变量序列
XC2 = df.iloc[133:166, 1].tolist()
XC3 = df.iloc[133:166, 2].tolist()

#B5
orders = [0.38748708, 0.57607172, 1]
lamda = [0.001, 0.49886868, 0.91910718]
'''orders = [1.02198014, 1.02198014, 1.02198014]
lamda = [0.22493405, 0.22493405, 0.22493405]
#B6
orders = [1,     0.64404203, 1 ]
lamda = [0.98470547, 0.001,  1.]
orders = [1.0338691, 1.0338691, 1.0338691]
lamda = [0.96936292, 0.96936292, 0.96936292]'''

#预测的个数和真实值
h = 33
forezsz = df.iloc[133:166, 3].tolist()
allX1 = df.iloc[:, 3].tolist()
N = 3
...
for k in range(2, len(x1_ago) + 1):
    ...
    # 计算s2
    s2 += μ3 * (k - 1) + ((k ** 2 - 1) / 2) * μ4 + ((k ** 3 - 1) / 3) * μ5 + ((k ** 4 - 1) / 4) * μ6
