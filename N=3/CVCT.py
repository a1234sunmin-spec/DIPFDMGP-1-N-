...
df = pd.read_excel('D:/研究生/专业学习/灰色预测/要写/锂电池/NASAlidianchi/NASA.xlsx', sheet_name='B6归一化')
# 1.这里将 x1 作为特征序列 x2,x3,x4,x5作为相关因素序列。19个拟合，5个预测
X1 = df.iloc[:133, 2].tolist()
X2 = df.iloc[:133, 0].tolist()
X3 = df.iloc[:133, 1].tolist()
#测试预测值数据 自变量序列
XC2 = df.iloc[133:166, 0].tolist()
XC3 = df.iloc[133:166, 1].tolist()
'''#B5
orders = [0.996474 ,  0.80474823, 0.55733854]
lamda = [0.52287439, 0.22028804, 0.70527038]'''
#B6
orders = [1.13342021, 0.00398153, 0.00062096]
lamda = [0.03758916, 0.08242937, 0.52236787]

#预测的个数和真实值
h = 33
forezsz = df.iloc[133:166, 2].tolist()
