...
df = pd.read_excel('D:/研究生/专业学习/灰色预测/要写/锂电池/NASAlidianchi/NASA.xlsx', sheet_name='B5归一化')
# 1.这里将 x1 作为特征序列 x2,x3,x4,x5作为相关因素序列。19个拟合，5个预测
X1 = df.iloc[:133, 1].tolist()
X2 = df.iloc[:133, 0].tolist()
X3 = df.iloc[:133, 2].tolist()
#测试预测值数据 自变量序列
XC2 = df.iloc[133:166, 0].tolist()
XC3 = df.iloc[133:166, 2].tolist()
#B5
orders = [1.00612283, 0.10545626, 0.47859944]
lamda = [0.82188488, 0.35433293, 0.65958357]
'''#B6
orders = [0.37739196, 0.39362673, 0.00801042]
lamda = [0.000187,   0.00262071, 0.55244777]'''


#预测的个数和真实值
h = 33
forezsz = df.iloc[133:166, 1].tolist()
allX1 = df.iloc[:, 1].tolist()
N = 3
