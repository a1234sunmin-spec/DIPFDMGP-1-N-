...
#B5
'''orders = [1.00019532, 0.37486862, 1.02993954]
lamda = [0.9208594,  0.03735881, 0.96962311]'''
#B6
'''orders = [0.99537115, 0.26351108, 0.85898911]
lamda = [0.73056686, 0.14938125, 0.09877649]'''
#B6
orders = [0.60812746, 0.58782872, 0.001]
lamda = [0.001 ,     0.76170925, 0.20797057]
...
for k in range(2, len(x1_ago) + 1):
    ...
    # 计算s2
    s2 += μ3 * (k - 1) + ((k ** 2 - 1) / 2) * μ4 + ((k ** 3 - 1) / 3) * μ5 + ((k ** 4 - 1) / 4) * μ6 + ((k ** 5 - 1) / 5) * μ7
