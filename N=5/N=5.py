...
#B5
orders = [0.4862875,  0.88714244, 0.91457827]
lamda = [0.001, 0.64521687, 1]
#B6
'''orders = [0.47292398, 0.66354591, 0.42683998]
lamda = [0.001, 0.35936112, 0.17331537]'''
#B6
'''orders = [0.19932529, 0.001,      0.08104577]
lamda = [0.001,  0.70483067, 0.21717829]'''

#预测的个数和真实值
h = 33
forezsz = df.iloc[133:166, 0].tolist()
...
for k in range(2, len(x1_ago) + 1):
    ...
    # 计算s2
    s2 += μ3 * (k - 1) + ((k ** 2 - 1) / 2) * μ4 + ((k ** 3 - 1) / 3) * μ5 + ((k ** 4 - 1) / 4) * μ6 + ((k ** 5 - 1) / 5) * μ7 + ((k ** 6 - 1) / 6) * μ8
